...
with RECURSIVE AncestryTree (pnc_stdy_smry_id, ancestor, lvl) as (
  		  	select 
...
with m1 (pnc_stdy_smry_id, pnc_ancestor_id) as 
  (
  ...
  update @pnc_smrypth_fltr m
  set tx_path_parent_key = m1.pnc_ancestor_id
  from m1
  where 
...
--update path (tx_stg_cmb_pth with modified_path in the recursive query) in #_pnc_smrypth_fltr
  WITH RECURSIVE t1(job_execution_id, combo_id, current_path1, pnc_stdy_smry_id, parent_key, modified_path, Lvl, depthOrder) AS (
        SELECT
      ...
      m1(rnum, combo_id, modified_path, lvl, current_path1, pnc_stdy_smry_id, parent_key, depthOrder) as (
...
update @pnc_smrypth_fltr m
set tx_stg_cmb_pth = m1.modified_path
from m1
where
  m.pnc_stdy_smry_id = m1.pnc_stdy_smry_id
  and m.job_execution_id = @jobExecId;
...
WITH RECURSIVE t1(combo_id, current_path1, pnc_stdy_smry_id, parent_key, modified_path, Lvl, depthOrder, job_execution_id) AS (
        SELECT 
          tx_stg_cmb                            as combo_id
          ,tx_stg_cmb_pth                       as current_path1
          ,pnc_stdy_smry_id                     as pnc_stdy_smry_id
...
WITH RECURSIVE t1(combo_id, current_path1, pnc_stdy_smry_id, parent_key, modified_path, modified_concepts, Lvl, depthOrder, job_execution_id) AS (
        SELECT 
...
(select JSON from (
	select distinct individualResult.table_row_id,
		string_agg(JSON::character varying, '') AS JSON
